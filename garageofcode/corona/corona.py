...
data_dir = "/home/jdw/garageofcode/data/kaggle/corona/"
res_dir = "/home/jdw/garageofcode/results/corona/"
num_cases_data = os.path.join(data_dir, "covid_19_data.csv")
case_data = os.path.join(data_dir, "COVID19_open_line_list.csv")
...
def estimate_unconfirmed():
    ...
    for country in ["Mainland China", "Sweden", "US", "France", "Germany", "Italy", 
                    "Iran", "UK", "South Korea", "Netherlands", 
                    "Norway", "Belgium", "Spain", "Switzerland", "Japan"]:
        df_country = df[df["Country/Region"] == country]
        day2case = df_country.groupby(["ObservationDate"])["Confirmed"].sum()
        ...
        plt.title("Estimated unconfirmed {}".format(country))
        plt.xlabel("Days")
